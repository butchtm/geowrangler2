# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/10_vector_to_raster_mask.ipynb.
...
def generate_mask(
    ...
    masks, grids = [], []
    for i, (_, x) in enumerate(gdf.iterrows()):

        if labels_column in x:
            value = label_values[x[labels_column]]
        else:
            value = nodata
        xdf = gpd.GeoDataFrame(pd.DataFrame(x).T, geometry='geometry', crs=raw.crs)
        xdf_json = xdf.to_json()
        gdf_json = json.loads(xdf_json)
        feature = [gdf_json["features"][0]["geometry"]][0]
        masks.append((feature, value))
        grids.append((feature, i))
        

    raster_masks = rio.features.rasterize(
        ((g, v) for (g, v) in masks),
        out_shape=src.shape,
        transform=src.transform,
    ).astype(rio.uint16)

    raster_grids = rio.features.rasterize(
    ...
    with rio.open(output_file, "w", **out_meta) as dst:
        dst.write(raster_masks, indexes=1)
        dst.write(raster_grids, indexes=2)

    if plot:
        f, ax = plt.subplots(1, 3, figsize=(15, 15))
        gdf.plot(ax=ax[0])
        rio.plot.show(src, ax=ax[1], adjust=None)
        rio.plot.show(raster_masks, ax=ax[2], adjust=None)
        ...
        plt.show()
    src.close()
    return raster_masks, raster_grids, label_values
